...
const PickUpButton: FC<{ playerId?: string }> = ({ playerId }) => {
  ...
  const isTableEmpty = useSelector(
    (state: RootState) => state.gameSlice.data.table.length === 0
  );
  const { table } = useSelector((state: RootState) => state.gameSlice.data);
  const [disabled, setDisabled] = useState(true);
  const areAllCardsBeaten = table
    .map((cardPair) => cardPair[1])
    .every((card) => !!card);
  useEffect(() => {
    setDisabled(areAllCardsBeaten || isTableEmpty);
  }, [areAllCardsBeaten, isTableEmpty]);
  const onClick = () => {
